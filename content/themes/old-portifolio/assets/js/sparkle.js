...
$.fn.sparkle = function(options) {
  ...
  return this.each(function(k, v) {
    ...
    var settings = $.extend({
      ...
      color: "#FFFFFF", count: 30, overlap: 0, speed: 0.1, minSize: 4, maxSize: 7, direction: "both"
...
Sparkle.prototype = {
  ...
  "update": function() {
    ...
    this.anim = window.requestAnimationFrame(function(time) {
      ...
      if (_this.fade) {
        _this.fadeCount -= 1;
        if (_this.fadeCount < 0) {window.cancelAnimationFrame(_this.anim);
          _this.$canvas.hide();} else {_this.update();}
      } else {_this.update();}
  ...
  "resize": function($parent) {
    ...
    // something has moved.
    if (this.isSingleton) {this.$canvas.css({
